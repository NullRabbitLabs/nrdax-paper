...
**Registry version**: v0.2, snapshot 2026-07-24
...
We are deliberately careful about what that recurrence shows. The registry records no implementation lineage, so counting distinct chains overstates independence: a chain that embeds libp2p is recorded under its own name. Working the headline case by hand, `NRDAX-T0100`'s nine chain deployments reduce to **four independently written handshake stacks** - RLPx, libp2p Noise, litep2p Noise and Solana's TPU QUIC - with three deployments sharing go-ethereum's RLPx and four sharing libp2p's. Four remains a real result for a mechanism with an obvious defence, and it is not nine. The corpus cannot be corrected the same way, because for most techniques the recorded data does not name the stack.
...
Supporting data: [`data/`](data/) - the first-pass audit (`mechanism-audit.py`, `mechanism-audit.csv`), the final per-technique assignment (`classification.py`), and the registry snapshot every figure was computed from (`registry-snapshot-2026-07-24.json`). The two audit files are also the record of how the axes were derived; see section 3.2.5.
