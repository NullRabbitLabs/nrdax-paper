...
We initially expected the remaining work to be curation rather than engineering, and it is not. Of the 23 multi-target techniques - the only ones where independence changes a claim - **exactly one has every instance's stack determinable from the published record, and it is `NRDAX-T0100`, the one already curated.** Twelve have none determinable and ten are partial.

The obstruction is structural rather than clerical. Lineage is recoverable from a primitive identifier only when that identifier names an implementation, and most name a protocol: `substrate_h2_rapid_reset` says HTTP/2 but not which HTTP/2 library, and independence is a question about implementations. `NRDAX-T0100`'s identifiers happened to name implementations - `bsc_rlpx_auth_flood`, `bor_rlpx_auth_flood`, `substrate_litep2p_noise_preauth_flood` - which is why that case could be worked at all. That was luck, not a property of the naming convention.

Curating the rest therefore requires the reproduction bundles, which record what was actually on the wire and are not published (section 9.6). It is a data-recovery task on unpublished artefacts, not a labelling exercise, and we have not done it. Until it is done, `NRDAX-T0100` is the only technique in this registry with an independence count rather than a lower bound.
...
Most of that gap is want of evidence: 303 of the 324 have no reproduced instance, so there is no mechanism to read. But **7 of them do have a reproduced instance, are in scope, and are simply not classified yet** (`NRDAX-T0009`, `NRDAX-T0096`, `NRDAX-T0110`, `NRDAX-T0111`, `NRDAX-T0332`, `NRDAX-T0338`, `NRDAX-T0381`). They arrived carrying one of the producing pipeline's coarse class labels rather than a fine-grained one, so they fell outside the slice the reclassification examined. We described this in an earlier draft as an oversight in our procedure rather than a limit on the evidence. That was wrong, and checking it produced the finding in section 8.5.1: **all seven have no mechanism text**. Their `mechanism` field repeats the display name, 27 to 41 characters against a median of 200 across the classified corpus. A mechanism is read off the reproduction, and for these seven there is nothing recorded to read. Classifying them would mean inferring a mechanism from a title, which is the inference-from-labels failure this classification exists to correct, and we would be committing it in the paper that criticises it. The gap is therefore not closeable without recovering the mechanism from the reproduction bundles, which are not published (section 9.6).

Recovering those seven is the first item of work after this draft, and it is a data-recovery task rather than a classification one. The identifiers are listed so anyone can confirm when it has been done.
...
### 8.5.1 The corpus behind the classified slice is weaker than the classified slice

Two measurements, both taken while checking the paragraph above, and neither previously reported.

**313 of 421 techniques carry no usable mechanism text.** The `mechanism` field repeats the title, or is short enough to be a title, rather than describing what the attack does. The classified slice is not where this bites: only 3 of the 97 are affected, and the median classified mechanism runs to 200 characters. But 10 of the 118 reproduced techniques are affected, and among the seven reproduced-but-unclassified it is all seven. The classification rests on a small, well-documented core sitting inside a corpus that is largely a list of names.

**34 CVE and GHSA identifiers are referenced by more than one technique.** For 19 of them the pattern is a curated technique sharing an identifier with a pending technique that has no mechanism text - roughly 7% of the 286 known-only records. `NRDAX-T0111` and `NRDAX-T0112` both carry CVE-2023-26964, and the first has no mechanism text and a primitive named for a different attack entirely.

We report the second as a signal and not a verdict, because sharing an identifier is not duplication. One advisory can describe genuinely distinct mechanisms, and this corpus contains examples: `NRDAX-T0320` and `NRDAX-T0329` share CVE-2025-26819 and classify as `connection_exhaustion` and `response_amp` respectively, which are different mechanisms by any reading. **We have not adjudicated the 19 individually.** Until someone does, the published total of 421 and the known-only figure of 286 should be read as upper bounds that may over-count by up to 19.

Neither measurement changes a classified count. Both say the same thing about the rest: the registry's reach exceeds its documentation, and the parts of it this paper leans on are the parts that are documented.
