...
Every figure below is computed from the pinned snapshot of 2026-07-24 in `data/` and can be checked against the live API. We have tried to state each limitation at the strength it actually has, rather than in the softened form that would be easier to publish.
